...
if __name__ == "__main__":
    ...
    test_data = [
        {
            "summary": "Person picks up a coffee mug from the table",
            "persons": ["person A"],
            "objects": ["coffee mug", "table"],
            "dialogue": ["I need my coffee"]
        },
        {
            "summary": "Person puts down the mug and picks up a book",
            "persons": ["person A"],
            "objects": ["mug", "book"],
            "dialogue": ["Time to read"]
        },
        {
            "summary": "Another person enters the room and waves",
            "persons": ["person B"],
            "objects": ["door"],
    ...
    bm25 = BM25Index()
    for node in nodes:
        bm25.add_document(node.node_id, node.get_searchable_text())
    
    results = bm25.search("coffee mug", top_k=3)
    print(f"Query: 'coffee mug'")
    for doc_id, score in results:
        print(f"  Node {doc_id}: {score:.4f}")
    
    results = bm25.search("person enters room", top_k=3)
    print(f"Query: 'person enters room'")
    for doc_id, score in results:
        print(f"  Node {doc_id}: {score:.4f}")
    ...
    print(f"Statistics: {unified.get_statistics()}")
    
    sparse_results = unified.search_sparse("coffee")
    print(f"Sparse search 'coffee': {sparse_results}")
